...
const ManageCommunity = ({ selectedCompanyId }) => {
    ...
    const handleConfirmRemove = async () => {
        try {
            // await removeUserFromCommunity(selectedCompanyId, userToRemove);
            // Update the local state to reflect the removal
            const updatedUsers = response.users.filter(user => user.handle !== userToRemove);
    ...
    const table1Data = response.users.map((user, index) => ({
        Temp: "", // If Temp is not needed, consider removing it or replacing it with relevant data
        Username: <Link to={`/explore?username=${user.handle}`} style={{ textDecoration: 'none', color: 'inherit' }}>{user.handle}</Link>,
        Name: user.user, // Assuming "user" field contains the name
        Points: user.RewardPoints,
        Remove: <Button onClick={() => handleRemoveClick(user.handle)} children="X" />,
    }));
